...
def data_viz(df):
    ...
    for idx, col in enumerate(df.select_dtypes(include=[object, np.bool]).keys()):
...
def data_viz_target(df, target_feat):
	...
	for c in list(df.select_dtypes(include=[np.number]).keys()):
	    print(c)
	    df.hist(column = c, by=target_feat, figsize=(10, 4), sharey=True, sharex=True, layout = (1, num_target_levels), density = True)
	    plt.show()
	
	for c in list(df.select_dtypes(include=[object]).keys()):
...
def visualize_tree(tree, feature_names, fileName="dt.png", label = 'all', impurity = True):
    ...
    with open("dt.dot", 'w') as f:
        export_graphviz(tree, out_file=f,
                        feature_names=feature_names, label=label, impurity = impurity)
